...
if [ "$APPCENTER_XAMARIN_CONFIGURATION" == "Release" ];then
    ...
    appcenter test run uitest --app "CDA-Global-Beta/GitTrends-Android-Debug" --devices "CDA-Global-Beta/google-play-screenshots" --app-path $APKFile --test-series "app-store-screenshots" --locale "en_US" --build-dir $UITestBuildDir --uitest-tools-dir $TestCloudExeDirectory --async
fi
